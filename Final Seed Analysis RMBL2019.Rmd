...
cat("ABUNDANCE", "\n", sep = "")
poll.abund.mod = glmmTMB(abundance ~ plot + (1|date) + (1|site), family = nbinom2(), data = visitdat.summ)
poll.abund.mod.aov = Anova(poll.abund.mod)
poll.abund.mod.aov

cat("\n", "\n", "SPECIES RICHNESS", "\n", sep = "")
poll.rich.mod = glmmTMB(richness ~ plot + (1|date) + (1|site), family = poisson, data = visitdat.summ)
poll.rich.mod.aov = Anova(poll.rich.mod)
poll.rich.mod.aov

```
As hypothesized, we do not see significant effects of our phenology acceleration manipulation on pollinator species richness or abundance, when assessed within site-week combinations. 
...
model assessments looks very clean for both models.

### community similarity pollinators

use the `adonis2` function in the `vegan` package to assess differences in Morisita-Horn similarity. Do this using sample dates nested within sites as `strata` within which permutations are generated. It looks like we should (??) be able to do this by specifying the `strata` via the [`Permute` package](https://cran.r-project.org/web/packages/permute/vignettes/permutations.html):

> The highest level of the hierarchy is the *block* level. Blocks restrict permutation of samples to within the levels of this factor; samples are never swapped between blocks.

>The *plot* level sits below blocks. Plots... group samples in the same way as blocks. As such, some permutation designs can be initiated using a factor at the plot level or the same factor at the block level. 

... but need to figure out how to specify this design
