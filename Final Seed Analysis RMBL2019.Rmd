...
```{r models of community-wide pollen limitation updated}
...
Following the convention set above of not including species as a main effect but instead assessing the interaction between phenology acceleration treatment and species. This is conservative here as it maximizes power to find differences in the main effect of phenology acceleration. 
...
# model
resource.limit.mod = glmmTMB(totalseeds ~ phenshift.binary/species + 
                  (1|site/site.plot), family = nbinom2, ziformula = ~1,
                  data = pollimdat.rl, control=glmmTMBControl(optimizer=optim, optArgs=list(method="BFGS")))
...
Next, we assess each species separately:
...
resource.limit.Denu = glmmTMB(totalseeds ~ phenshift.binary + 
                  (1|site/site.plot), family = nbinom2, ziformula = ~1,
                  data = pollimdat.Denu, control=glmmTMBControl(optimizer=optim, optArgs=list(method="BFGS")))
...
## failed model with ZI:
# resource.limit.Lala = glmmTMB(totalseeds ~ phenshift.binary + 
#                   (1|site/site.plot), family = nbinom2, ziformula = ~1,
#                   data = pollimdat.Lala, control=glmmTMBControl(optimizer=optim, optArgs=list(method="BFGS")))
...
resource.limit.Popu = glmmTMB(totalseeds ~ phenshift.binary + 
                  (1|site/site.plot), family = nbinom2, ziformula = ~1,
                  data = pollimdat.Popu, control=glmmTMBControl(optimizer=optim, optArgs=list(method="BFGS")))
...
looks problematic in terms of residuals vs. predicted values; stick to reporting the simpler model with 2- and 3-way interactions.
...
# cleanup
# rm(simout.rl, resource.limit.mod)
...
# cleanup
# rm(simout.rl, resource.limit.Denu, resource.limit.Lala, resource.limit.Popu)


```

all three look good; in particular no issue with lack of zero-inflation in Popu.
