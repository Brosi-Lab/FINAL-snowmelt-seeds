...
Packages <- c("knitr", "kableExtra", "glmmTMB", "fitdistrplus", "evaluate",
              "reshape2", "vegan", "car", "ggpubr", "broom.mixed", "mclogit", 
              "DHARMa", "gridExtra", "renv", "hillR", "tidyverse")
...
four datasets to import:

* seed data
    + most of the relevant data are included in here; 1155 rows in the original dataset
* snowmelt data
    + one data point per plot (16 plots, 8 sites each with a control and a snowmelt acceleration plot); data for day of year on which meltout occurred and acceleration of the snowmelt plot relative to the control
...
# flower visitation data raw
visitdat = read.csv("data/snowmelt_network.csv")
...
# fix date column name and make it a date object
names(visitdat)[1] <- "date"
visitdat$date <- as.Date(visitdat$date, format = "%m/%d/%y")
visitdat$year <- year(visitdat$date)
# filter to only include 2019
visitdat <- visitdat %>% filter(year == 2019)

# fix site names
visitdat$site = str_replace(visitdat$site, "pfeiler", "Pfeiler")
visitdat$site = str_replace(visitdat$site, "Wash 3C", "WG3C")
visitdat$site <- str_replace_all(visitdat$site," ","")

# remove Kaysee's data
visitdat <- visitdat %>% filter(plot != "Kaysee")

# sampling of Rustler's Gulch: some samples mislabeled as 8/24
# should be 8/23
datelist = which(visitdat$date=="2019-08-24")
visitdat$date[datelist] = "2019-08-23"

# no samples before 6-19 have both control and manip sampled
visitdat = visitdat %>% filter(date > "2019-06-18")

# Rustler's Gulch: 6/26 sample does not have data for Control; remove
visitdat = visitdat %>% filter(!date == "2019-06-26")
...
Here's a plot of the day of years that the snowmelt-accelerated plots melted out, with sites sorted by snowmelt day in the control plots:
...
plotsnow = ggplot(snowmeltdat, aes(y=sitesort, x=meltday, group = site, 
  ...
  theme_light() +
  xlab("day of year snow free") + ylab("sites") +
...
The objective here is just to assess if there are differences in the pollinator communities between snowmelt accelerated and control plots. Our hypothesis is that there will be no difference, because pollinators range over much larger areas than the size of the snowmelt plots.

### data prep pollinator comm data prep

* remove `BellviewBench` and `403Bench` sites (see "Community Fecundity" section)
* for clarity, strip down the columns to only those needed
* summarize dataframe at the site by week level to include visit number (abundance) and species richness
    + we can make direct comparisons of species richness because of perfectly balanced sampling


```{r data prep pollinator comm analyses}

# remove unused sites
site.list = c("BellviewBench", "403Bench")
visitdat = visitdat %>% filter(!(site %in% site.list))
# remove unused columns
visitdat = visitdat %>% dplyr::select(date, site, plot, specimen_number, gross_ID)

# remove blank visitor rows
visitdat = visitdat %>% filter(gross_ID != "")

# summarize by site-week
visitdat.summ = visitdat %>% group_by(site, plot, date) %>% summarize(richness = n_distinct(gross_ID), abundance = n())

## only if collapsing the gross_IDs into newIDs (as below for community composition)
## if doing this need to collapse the gross_IDs first!
# visitdat.summ = visitdat %>% group_by(site, plot, date) %>% summarize(richness = n_distinct(newID), abundance = n())

# check to see if control and manip at each site were each sampled on the same day
visit.check = visitdat.summ %>% group_by(date, site, plot) %>% summarize(n = n())
visit.check = pivot_wider(visit.check, names_from = plot, values_from = n)

# looks good

# clean up
rm(site.list, visit.check)

```

### analyses pollinator richness abundance
...
with `site` and `date` as crossed random intercepts. For both response variables, tried first with negative binomial errors; got a convergence problem with richness so reverted to Poisson (conducted model assessment below, no issues).

```{r pollinator richness abundance analyses}

cat("ABUNDANCE", "\n", sep = "")
cat("by site by week:", "\n", sep = "")
poll.abund.wk.mod = glmmTMB(abundance ~ plot + (1|date) + (1|site), family = nbinom2(), data = visitdat.summ)
poll.abund.wk.mod.aov = Anova(poll.abund.wk.mod)
poll.abund.wk.mod.aov

cat("\n", "by site only:", "\n", sep = "")
poll.abund.mod = glmmTMB(abundance ~ plot + (1|site), family = nbinom2(), data = visitdat.summ)
poll.abund.mod.aov = Anova(poll.abund.mod)
poll.abund.mod.aov

cat("\n", "\n", "SPECIES RICHNESS", "\n", sep = "")
cat("by site by week:", "\n", sep = "")
poll.rich.wk.mod = glmmTMB(richness ~ plot + (1|date) + (1|site), family = poisson, data = visitdat.summ)
poll.rich.wk.mod.aov = Anova(poll.rich.wk.mod)
poll.rich.wk.mod.aov

cat("\n", "by site only:", "\n", sep = "")
poll.rich.mod = glmmTMB(richness ~ plot + (1|site), family = poisson, data = visitdat.summ)
poll.rich.mod.aov = Anova(poll.rich.mod)
poll.rich.mod.aov


```
As hypothesized, we do not see significant effects of our phenology acceleration manipulation on pollinator species richness or abundance, when assessed either within site-week combinations, or by site alone. Surprisingly for species richness, the *p*-values are the same for both site-week and site, but I double-checked the code, and this is just coincidence.

### model assessment pollinator comm

```{r model assessment pollinator comm}

# ABUNDANCE
cat("ABUNDANCE", "\n", sep = "")
cat("by site by week:", "\n", sep = "")
# simulate residuals using DHARMa
simoutpollabund.wk <- simulateResiduals(fittedModel = poll.abund.wk.mod, plot = F)
plot(simoutpollabund.wk)
testDispersion(simoutpollabund.wk, plot = F)

cat("\n", "by site only:", "\n", sep = "")
# simulate residuals using DHARMa
simoutpollabund <- simulateResiduals(fittedModel = poll.abund.mod, plot = F)
plot(simoutpollabund)
testDispersion(simoutpollabund, plot = F)
...
cat("by site by week:", "\n", sep = "")
# simulate residuals using DHARMa
simoutpollrich.wk <- simulateResiduals(fittedModel = poll.rich.wk.mod, plot = F)
plot(simoutpollrich.wk)
testDispersion(simoutpollrich.wk, plot = F)

cat("\n", "by site only:", "\n", sep = "")
# simulate residuals using DHARMa
simoutpollrich <- simulateResiduals(fittedModel = poll.rich.mod, plot = F)
plot(simoutpollrich)
testDispersion(simoutpollrich, plot = F)
...
model assessments looks very clean for all four models.

### community similarity pollinators

steps are:

1. create site-by-species matrix from visitation data
1. calculate site-by-site dissimilarity matrix with Morisita-Horn similarity (in this case, using M-H not because of unbalanced sampling, but instead because of its robustness to incomplete sampling)
1. use `adonis2` to run a permutational MANOVA to statistically assess differences between snowmelt and control

Before we do this, want to combine some of the `gross_ID` categories:

* all *Bombus* into one category
* small & large *Megachile*
* all non-*Megachile* solitary bees (big and small)
* white and blue butterflies
* "unknown" = "Unknown"

In addition, we will want to remove any weeks from a given site---from both control and manip---for which there are fewer than three observations in either control or manip as these lead to issues with calculating community similarity.

Note that we cannot run the `adonis2` analysis using sample dates nested within sites as `strata`, because we need multiple samples within each strata, and we just have one sample per week. So this analysis has to be done at the `site` level.

```{r community similarity pollinators}

# 0. data prep; merge gross_ID categories
# all Bombus together
visitdat$newID = visitdat$gross_ID
indexy = which(str_detect(visitdat$gross_ID, "Bombus"))
visitdat$newID[indexy]="Bombus"
# all Megachile together
# note: doing Megachile before solitary, order matters
indexy = which(str_detect(visitdat$gross_ID, "megachile"))
visitdat$newID[indexy]="Megachile"
# all solitary non-Megachile together
indexy = which(str_detect(visitdat$gross_ID, "solitary"))
visitdat$newID[indexy]="Solitary bee"
# all butterflies
indexy = which(str_detect(visitdat$gross_ID, "butterfly"))
visitdat$newID[indexy]="Butterfly"
# all unknowns (change capitalization)
indexy = which(str_detect(visitdat$gross_ID, "unknown"))
visitdat$newID[indexy]="Unknown"
visitdat$newID = factor(visitdat$newID)

# 0.5 remove site-weeks that have fewer than 3 obs in either control or manip
# first find which ones meet that description:
visit.assess <- visitdat %>%
  group_by(site, plot, date) %>%
  tally()

indexy = which(visit.assess$n < 3)
dates = unique(visit.assess$date[indexy])
visitdat = visitdat %>% filter(!(date %in% dates))

# # check to see that it worked--YES
# # all values >= 3
# visit.assess <- visitdat %>%
#   group_by(site, plot, date) %>%
#   tally()
# which(visit.assess$n<3)

rm(visit.assess, dates)

# 1. create site-by-species matrix:
# first create a weighted edge list 
# plants in one col, pollinators in another, count in a third ("n"):
dat.weighted <- visitdat %>%
  group_by(site, plot, date, newID) %>%
  tally()

# convert weighted edge list into a network with `pivot_wider`:
site.species.mat <- dat.weighted %>%
  pivot_wider(id_cols = c("site", "plot", "date"), names_from = newID,
              values_from = n,
              values_fill = 0)

# 2. calculate Morisita-Horn similarity
morisita.poll = vegdist(site.species.mat[,-1:-3], method = "morisita")

# 3. run `adonis2`
# start by specifying permutations:
perm <- with(site.species.mat[,1:3], how(nperm = 999, blocks = site))

# adonis2:
poll.comm.sim = adonis2(morisita.poll ~ plot, data = site.species.mat, permutations = perm, na.rm = TRUE, method = "morisita")
print(poll.comm.sim)
# confirmed from output that "Blocks: site" (which is what we want)

# clean
rm(dat.weighted, poll.comm.sim, morisita.poll, site.species.mat)

```
As hypothesized, no difference in community similarity of pollinators at the site level driven by snowmelt manipulation.
...
Included in the model random effects specification: there are plots (control vs. manip) nested within each site; add these in as a random effect (random intercept), nested within site: `(1|site/site.plot)`
...
We display here only the final model, with the 3-way interaction; main effect of phenological shift; and a two-way interaction between phenological shift and plant species).
...
fecund.effect.size = 1- (phenfecsum$mean.seeds[2]/phenfecsum$mean.seeds[1])
...
The effect size of the reduction in fecundity is a `R round(fecund.effect.size,3)`% reduction in overall seed production in the accelerated plots relative to control. 
...
# cleanup from previous code chunk:
rm(fecund.effect.size)

# don't cleanup just yet from this chunk, need to combine plots first
...
# don't clean up `reddat` yet---used in the effect size of pollen limitation piece
## rm(reddat)

```

### models of community-wide pollen limitation initial
...
```{r models of community-wide pollen limitation, warning = F, message = F}
...
### updated models of community-wide pollen limitation

After plotting these models (below), it is clear that the hand-pollination treatments did not work for `Mefu` or for `Lile`; instead, the hand-pollinated plants always produced fewer seeds than the open-pollinated plants in both treatment and control plots. Since it was our experimental manipulation of these plants that failed, we should remove those species from this analysis as they could obfuscate results. This leaves us unfortunately with only 3 species, but still very much worth assessing those combined.

```{r models of community-wide pollen limitation updated, warning = F, messages = F}
...
# REMOVE MEFU AND LILE:
pollimdat.up = pollimdat %>% filter(species != "Mefu" & species != "Lile")

# model with 2- and 3-way interactions (doesn't include flower number):
# adding in the `control` parameter to avoid "non-positive-definite Hessian"
# ("BFGS" optimizer):
# https://cran.r-project.org/web/packages/glmmTMB/vignettes/troubleshooting.html
poll.limit.mod.up = glmmTMB(totalseeds ~ (plant.treat:phenshift.binary)/species + 
                  (1|site/site.plot), family = nbinom2, ziformula = ~1,
                  data = pollimdat.up, control=glmmTMBControl(optimizer=optim, optArgs=list(method="BFGS")))

# print results in Rmarkdown report
cat("\n model with 2- and 3-way interactions (doesn't include flower number)\n")
Anova(poll.limit.mod.up)

# model with 2- and 4-way interactions (includes flower number)
# tried the "BFGS" optimizer which was needed above; 
# but here throws an error, so not using
poll.limit.mod24.up = glmmTMB(totalseeds ~ (plant.treat:phenshift.binary)/(species:num.flowers) + 
                        (1|site/site.plot), family = nbinom2, ziformula = ~1,
                        data = pollimdat)
# print results in Rmarkdown report
cat("\n model with 2- and 4-way interactions (includes flower number)\n")
Anova(poll.limit.mod24.up)

## use `broom.mixed` package to save model summary as a table
# first model (2- & 3- way interactions)
aov.poll.limit.mod.up = Anova(poll.limit.mod.up)
aov.poll.limit.mod.up = tidy(aov.poll.limit.mod.up)
write.csv(aov.poll.limit.mod.up, file = "results/community-pollen-limitation-model-UPDATED-results.csv")

# second model (2- & 4- way interactions)
aov.poll.limit.mod24.up = Anova(poll.limit.mod24.up)
aov.poll.limit.mod24.up = tidy(aov.poll.limit.mod24.up)
write.csv(aov.poll.limit.mod24.up, file = "results/community-pollen-limitation-model-UPDATED-results-2-4.csv")
...
To test if there are differences in resource limitation between the control plots and the phenology-accelerated (manipulated) plots, we can compare the hand-pollinated (only) plants between the two groups. If there is no significant difference between the two, we do not have any evidence of resource limitation driven by our treatment (to be clear, that is *not* the same as saying that there is significant homogeneity between the two). 
By contrast, if we find a significant difference, then the treatment has driven changes in resource limitation. These could reduce resource limitation (i.e. benefit reproduction) theoretically in either the control or the phenology-accelerated plots.

Because we are zeroing in to see if there are differences among the hand-pollinated plants in control and manip, and we want to maximize our power to potentially find differences, we will use species as a random intercept (different plant species have different mean / maximum seed counts). Moreover, we expect that because all of these plants are hand-pollinated, that we will have fewer plants with zero whole-plant seed production, thus trying without a zero-inflation term at least to start. Will add one in if model assessment deems it necessary.

First we assess a model for all three of the hand-pollinated plant species pooled:

```{r resource limitation model pooled}

# limit to hand-pollinated data only
pollimdat.rl = pollimdat.up %>% filter(plant.treat == "open-hand")

# model
resource.limit.mod = glmmTMB(totalseeds ~ phenshift.binary +
                  (1|site/site.plot) + (1|species), family = nbinom2,
                  data = pollimdat.rl)

# display results
Anova(resource.limit.mod)

```
the lack of significance in the main effect (`phenshift.binary`) is consistent with no impact of our phenology acceleration manipulation on resource limitation.

Next, we assess each species separately; same as above but taking out the species-level random intercept:

```{r resource limitation model species specific}

# Denu (Delphinium nuttalianum)
pollimdat.Denu = pollimdat.rl %>% filter(species == "Denu")

# model
resource.limit.Denu = glmmTMB(totalseeds ~ phenshift.binary + 
                  (1|site/site.plot), family = nbinom2, data = pollimdat.Denu)

# display results
cat("\n", "=================================================", "\n", sep = "")
cat("## Delphinium nuttalianum resource limitation ##", "\n", sep = "")
Anova(resource.limit.Denu)

# =============
# Lale (Lathyrus lanzwertii)
pollimdat.Lala = pollimdat.rl %>% filter(species == "Lala")

# model
# initial attempt with zero-inflation failed to converge
# model without ZI:
resource.limit.Lala = glmmTMB(totalseeds ~ phenshift.binary + 
                  (1|site/site.plot), family = nbinom2, data = pollimdat.Lala)

# display results
cat("\n", "=================================================", "\n", sep = "")
cat("## Lathyrus lanzwertii resource limitation ##", "\n", sep = "")
Anova(resource.limit.Lala)

#===============
# Popu (Potentilla pulcherrema)
pollimdat.Popu = pollimdat.rl %>% filter(species == "Popu")

# model
resource.limit.Popu = glmmTMB(totalseeds ~ phenshift.binary + 
                  (1|site/site.plot), family = nbinom2, data = pollimdat.Popu)

# display results
cat("\n", "=================================================", "\n", sep = "")
cat("## Potentilla pulcherrema resource limitation ##", "\n", sep = "")
Anova(resource.limit.Popu)

```

No evidence of resource limitation driven by the phenology acceleration manipulation in any of the species.
...
## cleanup
rm(simoutpolllim, simoutpolllim24, poll.limit.mod, poll.limit.mod24, 
   aov.poll.limit.mod, aov.poll.limit.mod24)

```

looks problematic in terms of residuals vs. predicted values; stick to reporting the simpler model with 2- and 3-way interactions (rather than this model that includes 4-way interactions)

#### model assessment resource limitation


```{r model assessment resource limitation}

# pooled resource limitaiton model
simout.rl <- simulateResiduals(fittedModel = resource.limit.mod, plot = F)
plot(simout.rl)

# print dispersion and zero-inflation test results (don't plot for space)
testDispersion(simout.rl, plot = F)
testZeroInflation(simout.rl, plot = F)

# cleanup
rm(resource.limit.mod, simout.rl)

```

resource limitation model (pooled across species) looks fine

**species-specific models**
Now assess each of the species-specific models (note, Lale does not include zero-inflation because model did not converge)

resource.limit.Popu

```{r model assessment resource limitation species specific}

# Denu
cat("=================================================", "\n", sep = "")
cat("Delphinium nuttalianum resource limitation")
simout.Denu <- simulateResiduals(fittedModel = resource.limit.Denu, plot = F)
plot(simout.Denu)

# print dispersion and zero-inflation test results (don't plot for space)
testDispersion(simout.Denu, plot = F)
testZeroInflation(simout.Denu, plot = F)

#============================
# Lala
cat("=================================================", "\n", sep = "")
cat("Lathyrus lanzwertii resource limitation")
simout.Lala <- simulateResiduals(fittedModel = resource.limit.Lala, plot = F)
plot(simout.Lala)

# print dispersion and zero-inflation test results (don't plot for space)
testDispersion(simout.Lala, plot = F)
testZeroInflation(simout.Lala, plot = F)

#============================
# Popu
cat("=================================================", "\n", sep = "")
cat("Potentilla pulcherrema resource limitation")
simout.Popu <- simulateResiduals(fittedModel = resource.limit.Popu, plot = F)
plot(simout.Popu)

# print dispersion and zero-inflation test results (don't plot for space)
testDispersion(simout.Popu, plot = F)
testZeroInflation(simout.Popu, plot = F)

#============================

# cleanup
rm(resource.limit.Denu, resource.limit.Lala, resource.limit.Popu, simout.Popu,
   simout.Denu, simout.Lala, pollimdat.Denu, pollimdat.Lala, pollimdat.Popu)

```

all three look good.

#### model assessment pollen limitation UPDATED

This section reports on model validation for the pollen limitation models with "Lile" and "Mefu" removed (since the hand-pollination treatments did not work on those species)

Assess model residuals to see if the model meets basic statistical assumptions, using the `DHARMa` package. Also asssess dispersion (over- and under-dispersion) and zero-inflation. Trying this out for both models, those with 3-way and with 4-way interactions.

**updated model with 2- and 3- way interactions:**

```{r model assessment pollen limitation 23 updated}

# simulate residuals using DHARMa
# "simoutpollim" = simulation output, pollen limitation models
# (with 2- and 3-way interactions)
simoutpolllim <- simulateResiduals(fittedModel = poll.limit.mod.up, plot = F)
plot(simoutpolllim)
...
```
while plots aren't the cleanest, no obvious issues and dispersion test and overdispersion tests not significant---feel comfortable reporting on these models.

**updated model with 2- and 4- way interactions:**

again, without "Lile" and "Mefu"

```{r model assessment pollen limitation 24 updated}
# "simoutpollim24" = simulation output, pollen limitation models 
# (with 2- and 4-way interactions)
simoutpolllim24 <- simulateResiduals(fittedModel = poll.limit.mod24.up, plot = F)
plot(simoutpolllim24)
...
# cleanup
rm(simoutpolllim, simoutpolllim24, poll.limit.mod.up, 
    poll.limit.mod24.up, aov.poll.limit.mod.up, aov.poll.limit.mod24.up)

```

These do not look great (as with the models that included Lile and Mefu)---stick with the model with 2- and 3-way interactions.
...
#### UPDATED plot summarized community pollen limitation

* difference with the above is that this one does not include Mefu or Lile, since the hand-pollination experiments did not function as expected in those species
* here plotting the 2-way interaction between `plot.treat` (hand-pollinated vs. open-pollinated) and `phenshift.binary` (whether or not a plant in a plot had been experimentally phenologically advanced)
* this plot is to aid in the interpretation of the 2-way statistical interaction
* going to plot means only; the CIs are not likely to be informative here given potential differences in sites and species composition (as is typically the case with GLMMs)

```{r community pollen limitation plot updated, message = FALSE}

pollimsum.up = pollimdat.up %>% group_by(phenshift.binary, plant.treat) %>%
  summarise(mean.seeds = mean(totalseeds), sd.seeds = sd(totalseeds, na.rm = T), 
        n.plants = n()) %>% 
        mutate(ci.lower = mean.seeds - 1.96*(sd.seeds/sqrt(n.plants)),
          ci.upper = mean.seeds + 1.96*(sd.seeds/sqrt(n.plants)),
...
pollimplot.up = ggplot(pollimsum.up, aes(x = phenshift.binary, y = mean.seeds, 
                                   color = plant.treat)) +
  geom_point(aes(group = plant.treat), 
             position = position_dodge(width=0.2), size = 3) +
  geom_line(aes(group = plant.treat), position = position_dodge(width=0.2)) + 
...
# # (keep this commented out to prevent repeated saving on GitHub)
# # re-save when this changes, but only as needed
# ggsave(plot = pollimplot.up, width = 4, height = 4,
#        filename = "plots/aggregated-pollen-limitation-plot-updated.pdf")
# ggsave(plot = pollimplot.up, width = 4, height = 4,
#        filename = "plots/aggregated-pollen-limitation-plot-updated.jpg")

# don't cleanup yet; need to combine plots below

```

#### Effect sizes of pollen limitation

Need to be thoughtful about reporting effect sizes, because the effect stems from an interaction.

**Ultimately, the fecundity differences between control and phenology-accelerated plants---both aggregated and at the species level---are consistent with being driven _completely_ by pollen limitation and not by resource limitation.**

Here is more on that rationale, in the context of interpreting pollen limitation data:

* FIRST: resource limitation. One can examine the hand-pollinted data (slope of the line from control to manip) to see if there are differences in resource limitation, because these data are experimentally designed to show the case where there is *no* pollen limitation
    + if the hand-pollination line is flat (no difference between control and manip), then resource limitation is not at play
    + if by contrast the hand-pollinated treatment has higher fecundity in control, that would mean that there is a negative impact of the phenology acceleration on resource limitation (i.e. more resource limitation in accelerated plots)
    + similarly, if hand-pollinated fecundity is higher in the phenology acceleration treatment, then there is a positive impact of the phenology acceleration on resource limitation (i.e. less resource limitation in accelerated plots)
* SECOND: pollen limitation. If there is no difference between the hand-pollination fecundity and the open-pollination fecundity in either experimental treatment, that means that there is no pollen limitation.
    + if there is a difference between open and hand pollinated plants in any treatment, that means that there is some pollen limitation occurring
    + if there is no difference between open and hand in the control, that means that plants were not pollen limited in the control; but if that result is combined with a difference between open and hand in the phenology-accelerated treatment (as we found both overall and for *D. nuttalianum*), AND there is no resource limitation (flat line between hand-pollinated treatments in control and phenology manip) then we can say that the negative effects of the phenology manipulation are completely due to differences in pollen limitation
* if there were a difference between open and hand in both control and phenology manip, but the effect were bigger in phenology manip, then we could calculate the proportional difference between the two to get at the effect size of pollen limitation (how much *more* pollen limitation there was in treatment vs. control)---but again we do not have that situation here.

The code chunk below set to `"eval = F"` because following the above rationale, pollen limitation is responsible for all of the difference between control and phenology manip in all of the cases we found (both combined and species-by-species), including the positive effects of the phenology manipulations on *Lathyrus lanzwertii*.

```{r pollen limitation effect overall size, eval = F}

# code from the community-wide fecundity component above; altering to include only the three species of interest (Lathyrus, D. nutt., and Potentilla pulch)---implement this by excluding Lile and Mefu

# START by calculating the effect size from the "full" dataset used for the community fecundity analysis. Then calculate for the subset of data used for the pollen limitation analyses.

# FULL DATASET
# exclude Lile and Mefu:
reddat = reddat %>% filter(species != "Mefu" & species != "Lile")

# summarize data:
phenfecsum.effectsize = reddat %>% group_by(phenshift.binary) %>%
  summarise(mean.seeds = mean(totalseeds), sd.seeds = sd(totalseeds, 
                                na.rm = T), n.plants = n()) %>%  
          mutate(ci.upper = mean.seeds + 1.96*(sd.seeds/sqrt(n.plants)),
          ci.lower = mean.seeds - 1.96*(sd.seeds/sqrt(n.plants)),
...
# raw fecundity difference, control vs. manipulation:
fecund.diff = round(phenfecsum.effectsize$mean.seeds[1] - phenfecsum.effectsize$mean.seeds[2],2)

# proportional fecundity difference, control vs. manipulation:
prop.fecund.diff = round(fecund.diff/phenfecsum.effectsize$mean.seeds[1],3)

# POLLEN LIMITATION DATASEST
pollim.effect.prop = 1 - pollimsum.up$mean.seeds[3]/pollimsum.up$mean.seeds[1]

```

<!-- **TEXT COMMENTED OUT FOR NOW, PROBABLY DELETE** -->

<!-- For the three target species, not limiting our data to the pollen limitation subset, the effect size of the phenology acceleration manipulation is `R fecund.diff` fewer seeds in the manipulated plots relative to controls. This is a proportional difference of `R prop.fecund.diff`, which is very similar to the effect size of `R round(fecund.effect.size,3)`. -->

<!-- Because we are using a different subset of data for the pollen limitation analyses, it is probably better to compare proportional effects rather than raw effects. -->

<!-- `R round(pollim.effect.prop,3)`. -->
...
In these models, we want to use a slightly different dataset relative to those used in the community fecundity analyses. Here, we do *not* want to exclude site-species combinations for which there was no phenological advancement. The focal fixed effect is the snowmelt treatment, and the response is the proportion of seeds coming from the different plant species, so it is useful to know the outcome if some species did not change their flowering time (i.e., phenological advancement) and others did. As before, we are not including the two sites for which the snowmelt acceleration did not change flowering phenology for any plant species.
