...
# check to see if control and manip at each site were sampled on the same day
...
# some site-plot-date combos have `NA`s: address
# no samples before 6-19 have both control and manip sampled
visitdat.summ = visitdat.summ %>% filter(date > "2019-06-18")
# Rustler's Gulch: 6/26 sample does not have data for Control; remove
visitdat.summ = visitdat.summ %>% filter(!date == "2019-06-26")

# repeat check to be sure:
visit.check = visitdat.summ %>% group_by(date, site, plot) %>% summarize(n = n())
visit.check = pivot_wider(visit.check, names_from = plot, values_from = n)
# looks good
...
model assessments looks very clean for both.
