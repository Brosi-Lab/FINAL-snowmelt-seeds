...
In this analysis, we assess if the timing of snowmelt impacts seed set in montane plant communities, with a plant-community-level focus. We conducted an accelerated snowmelt experiment at RMBL, Colorado in summer 2019. We worked at 8 sites scattered across 2 valleys. In each site, we had paired control-manip plots, each 14 x 10 m. In manip plots, we accelerated snowmelt by solarizing the snow with black shade cloth. Snowmelt rate in control plots was unmanipulated. For the most part, accelerating snowmelt caused plant phenology to begin earlier in manipulated plots. 

We hypothesize that earlier snowmelt date could affect plant seed set through advances in flowering time that potentiate plant-pollinator phenology mismatches.

## TO-DOs

> stitch figures together; particularly figs 1a & 1b (overall snowmelt date and phenology acceleration by site)
...
Packages <- c("tidyverse", "knitr", "kableExtra", "glmmTMB", "fitdistrplus", 
              "reshape2", "vegan", "car", "ggpubr", "broom.mixed", "mclogit", "DHARMa") 

# this next line checks to see if all of the packages are installed
# if not, it installs them; note `installed.packages()` can be a little slow
# install.packages(setdiff(Packages, rownames(installed.packages())), repos = "http://cran.us.r-project.org")

# this line then loads all of the packages:
lapply(Packages, library, character.only = TRUE)

# suppress silly messages from dplyr about group-level summarizing:
...
# seed data raw
masterdat = read.csv("Masterseed2019.csv", stringsAsFactors = F, header=T)
# snowmelt dates data raw
snowmeltdat = read.csv("regressionmeltdays.csv", header = T)
# species-plot level phenological shift
phenoshift = read.csv("phenoshifted.csv", stringsAsFactors = F, header=T)
...
masterdat = masterdat[!is.na(masterdat$num.flowers),]
...
Before we fit any models, we want to see how the response variable (seed count) is distributed. It is a discrete count variable; in past data analyses, seed counts have been overdispersed and fit much better with a negative binomial distribution than with a Poisson, but worth a quick check.
...
# plot them:
cat("Poisson:\n") # this just prints the title before the plot
plot(isitgonnabepois)
cat("Negative Binomial:\n") # again, printing the title before the plot
plot(isitgonnabenbinom) # negative bionmial is the one!
...
The seed data generally fit a negative binomial distribution much better than a Poisson (no surprise). We will also assess overdispersion when actually running models.
...
# Plot
plotsnow = ggplot(snowmeltdat, aes(y=sitesort, x=meltday, group = site, color = plot.treat)) + 
  geom_point(size = 4) +
  geom_line(color = "black") +
  theme_light() +
  xlab("meltout day of year") + ylab("sites") +
  theme(axis.title.y = element_text(size = 12)) +
  theme(axis.text = element_text(size = 12)) +
  theme(strip.text.x = element_text(size = 12, colour = "black")) +
  scale_colour_manual(values=c("orangered3", "darkslategray2"))

plotsnow
#ggsave(plot = plotsnow, width = 6, height = 3, dpi = 300, filename = "plotsnow.pdf")

# cleanup
# includes removing entire `snowmeltdat` dataframe, not used again
# rm(plotsnow, snowmeltdat) 
...
meansbysite = masterdat %>% filter(plot.treat == "accel") %>% 
  group_by(site, species) %>% summarise(speciesmean = mean(phenshift, na.rm = T))
...
meansbysite$site <- factor(meansbysite$site, levels = c("Pfeiler", "BellviewBench", "403Bench", "RustlersGulch", "Meridian","AveryPicnic", "WG3C", "StupidFalls"))
...
meanplotbysite = meanplotbysite + theme(
...
# meanplotbysite = ggsave("meanphenoshiftplot.png", width = 5, height = 3)

# cleanup
# includes removing `phenoshift` dataset, not used again
rm(phenoshift, meanplotbysite, meansbysite)
...
Anova(comm.fec.3)
# don't cleanup / remove this model just yet (do it below after saving as .csv)

## use `broom.mixed` package to save model summary as a table
aov.comm.fec = Anova(comm.fec.3)
out.aov.comm.fec = tidy(aov.comm.fec)
# write.csv(out.aov.comm.fec, file = "community-fecundity-model-results.csv")

# cleanup
# don't cleanup `comm.fec.3` since we need to assess model assumptions
...
* three-way interaction between phenological acceleration, species, and flower number:
    + indicates that not only do different species respond differently to phenological acceleration overall, but also that they have different responses vis-a-vis flower number (in terms of how phenological acceleration changes the slope of the {flower number} - to - {whole plant fidelity} relationship
...
plot(simoutcommfec)
testDispersion(simoutcommfec)
testZeroInflation(simoutcommfec)
...
Looks great! No issues with distribution of residuals, dispersion, or zero-inflation. Carry on.
...
sppfecplot = ggplot(sumsppfec, aes(x=phenshift.binary, y=log(meanfecund), color = phenshift.binary)) +
  geom_point(size = 1.7) +
  geom_line(aes(group = species), color = "dark grey") +
  scale_colour_manual(values=c("darkslategray2", "orangered3")) +
  facet_wrap(~species, nrow = 1) +
  ylab("log(mean fecundity)") +
  xlab("\nphenological shift (presence/absence)") +
  ggtitle("Phenshift:Species, no CIs") +
  theme_light() +
  theme(axis.title.y = element_text(size = 12)) +
...
sppfecplotci = ggplot(sumsppfec, aes(x=phenshift.binary, y=log(meanfecund), color = phenshift.binary)) +
  ...
  geom_line(aes(group = species), color = "dark grey") +
  scale_colour_manual(values=c("darkslategray2", "orangered3")) +
  geom_errorbar(aes(ymin=log(lower.ci.fecund), ymax=log(upper.ci.fecund)), width=0.2, alpha = 0.5)+
  facet_wrap(~species, nrow = 1) +
  ylab("log(mean fecundity)") +
  xlab("\nphenological shift (presence/absence)") +
...
```{r Community seed - 3 way interaction, message = FALSE, warning = FALSE}

# Plot
plotcommunity2 = ggplot(commfecdat, aes(x=log(num.flowers), y=log(totalseeds+1), color = species)) +
...
# Plot
plotcommunity3 = ggplot(commfecdat, aes(x=log(num.flowers), y=log(totalseeds+1), color = species, linetype = phenshift.binary)) +
...
# Plot
plotcommunity1 = ggplot(commfecdat, aes(x=log(num.flowers), y=log(totalseeds+1), color = phenshift.binary)) +
  geom_point(data = commfecdat, aes(group = species), size = 1.5, alpha = 0.5) +
  geom_smooth(method = 'lm', se = F) +
  theme_light() +
  ylim(-0.1, 8) +
  ...
  theme(axis.text = element_text(size = 12)) +
  theme(strip.text.x = element_text(size = 12, colour = "black")) +
  scale_colour_manual(values=c("gray60", "orangered3"))


Fig3 = ggarrange(plotcommunity1, plotcommunity3,
          labels = c("A", "B"),
          ncol = 2, nrow = 1,
          align = "v")

Fig3
# ggsave(plot = Fig3, width = 10, height = 4, dpi = 300, filename = "plotcommunity.pdf")
rm(Fig3, plotcommunity1, plotcommunity2, plotcommunity3)

```
Shaded areas are standard errors.
When phenological change occurs, it tends to decrease seed set with one exception, Lala. 
* Decrease seed set may be more pronounced in Bost and Denu.
* However, Lala seed set is increased

Plotting the selfers vs non-selfers 3-way interaction.
...
# these models were tricky to specify syntactically...
...
## use `broom.mixed` package to save model summary as a table
# saving first model (2- & 3- way interactions only)
aov.poll.limit.mod = Anova(poll.limit.mod)
aov.poll.limit.mod = tidy(aov.poll.limit.mod)
write.csv(aov.poll.limit.mod, file = "community-pollen-limitation-model-results.csv")
...
* 2-way interaction indicates an overall effect of pollen limitation and phenological advancement
    + plot (next code chunk) suggests *greater* pollen limitation (i.e. more negative) under phenological advancement
    + highly significant ($p$ = 1.811e-08) when assessed in conjunction with the 3-way interaction
    + significant ($p$ = 0.01985) when assessed with the 4-way interaction
...
cat("\n model with 2- and 3-way interactions (doesn't include flower number)\n")
plot(simoutpolllim)
testDispersion(simoutpolllim)
testZeroInflation(simoutpolllim)
```
...
cat("\n model with 2- and 4-way interactions (does include flower number)\n")
plot(simoutpolllim24)

# # cleanup
# rm(simoutpolllim, simoutpolllim24, poll.limit.mod, 
#    poll.limit.mod24, aov.poll.limit.mod)
...
* going to plot means only; the CIs are not likely to be informative here given potential differences in sites and species composition (this is often the case with GLMMs)
...
pollimplot = ggplot(pollimsum, aes(x = phenshift.binary, y = mean.seeds, color = plant.treat)) +
  geom_point(aes(group = plant.treat), size = 3) +
  geom_line(aes(group = plant.treat)) + 
  # geom_errorbar(aes(ymin = ci.lower, ymax = ci.upper), width = 0.1) + 
  # scale_colour_manual(values=c("bisque4", "chartreuse2")) +
  xlab("phenological acceleration") + 
  ylab("mean whole-plant fecundity")

pollimplot # display in Rmarkdown report
# rm(pollimplot, pollimsum) # cleanup
...
Somewhat surprising that open-pollinated plants produced on average more seeds in the open treatment when not phenologically advanced (left-hand points), but that difference may not be statistically meaningful (again, confidence intervals are likely to be huge given site and species specific differences, and also unlikely to be helpful in interpretation here).
...
polllimplot = ggplot(sumxpoll, aes(x=phenshift.binary, y=log(meanfecund), color = plant.treat)) +
  ...
  ylab("log(mean fecundity)") +
  xlab("\nphenological shift (presence/absence)") +
  ggtitle("Planttreat:Phenshift:Species, no CIs") +
  theme_light() +
  theme(axis.title.y = element_text(size = 12)) +
  theme(axis.title.x = element_text(size = 12)) +
  theme(axis.text = element_text(size = 12)) +
  theme(strip.text.x = element_text(size = 12, colour = "black"))
polllimplot # display plot

# plot with confidence intervals:
polllimplotci = ggplot(sumxpoll, aes(x=phenshift.binary, y=log(meanfecund), color = plant.treat)) +
  geom_point(data = sumxpoll, size = 1.7, alpha = 0.8) +
  geom_line(aes(group = plant.treat)) +
  geom_errorbar(aes(ymin=log(lower.ci.fecund), ymax=log(upper.ci.fecund)), width=0.2, alpha = 0.5)+
  facet_wrap(~species, nrow = 1) +
...
Note that in the model specification, we set `dispersion = T` such that the model includes a specific estimate of the dispersion parameter, and thus overdispersion should not be an issue.
