...
plotsnow = ggplot(snowmeltdat, aes(y=sitesort, x=meltday, group = site, color = plot.treat)) + 
  geom_point(size = 4) +
  geom_line(color = "black", arrow = arrow(angle = 30, length = unit(0.09, "inches"), ends = "first", type = "open")) + 
  theme_light() +
...
# don't remove plot just yet, until it is combined with the other one
rm(snowmeltdat) 
...
rm(plotsnow, phenoshift, meanplotbysite, meansbysite, meltphenplot)
...
sppfecplot = ggplot(sumsppfec, aes(x=phenshift.binary, y=log(meanfecund), color = phenshift.binary)) +
  ...
  ggtitle("Phenology shift by species effects on fecundity") +
  theme_light() +
...
# # (keep this commented out to prevent repeated saving on GitHub)
# # re-save when this changes, but only as needed
# ggsave(sppfecplot, file = "plots/fecundity-by-species-phenology.pdf",
#    width = 5, height = 3)

# cleanup
rm(sumsppfec, sppfecplot, sppfecplotci)
...
```{r plot interactive effects of phenology shift on fecundity, message = FALSE, warning = FALSE}
...
```
Left panel (A) essentially shows the same thing as the aggregated plot, i.e. that phenological shifts overall reduce total fecunity; but shows it in interaction with flower number (agnostic to species identity)
